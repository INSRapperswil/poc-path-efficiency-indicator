...
def encodeNum(number, bitwidth):
    ...
    if number < 0:
        if number < -(2 ** (bitwidth - 1)):
            raise Exception(
                "Negative number, %d, has 2's complement representation that does not fit in %d bits"
                % (number, bitwidth)
            )
    ...
    if orig_number < 0:
        print(
            "CONVERT_NEGATIVE_NUMBER debug: orig_number=%s number=%s bitwidth=%d num_str='%s'"
